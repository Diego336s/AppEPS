...
import { cargarCitasConMedicosEspecialidades } from "../../Src/Services/RecepcionService";
...
export default function HistorialMedico() {
  ...
  useEffect(() => {
    ...
    const CargarCitas = async () => {
      try {
        const response = await cargarCitasConMedicosEspecialidades();
        if (response.message) {
        ...
        {filteredCitas.length > 0 ? (
          filteredCitas.map((item, index) => (
            <View key={index} style={styles.card}>
              ...
              <Text style={styles.text}>{item?.nombre_paciente} {item?.apellido_paciente}</Text>

              <Text style={styles.label}>Documento:</Text>
              <Text style={styles.text}>{item?.documento_paciente}</Text>
